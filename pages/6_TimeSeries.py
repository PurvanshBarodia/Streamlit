...
df = pd.read_csv(r'C:\Users\purva\Desktop\Streamlit\Data\AirPassengers.csv')
...
with tab2:
    ...
    st.code('from statsmodels.tsa.seasonal import seasonal_decompos')
...
with tab3:
    ...
    st.markdown("* :red[AR(p) Autoregression] – a regression model that utilizes the dependent relationship between a current observation and observations over a previous period. An auto regressive (AR(p)) component refers to the use of <span style='color:orange; font-weight: bolder'>past values</span> in the regression equation for the time series.\n * :green[I(d) Integration] – uses differencing of observations (subtracting an observation from observation at the previous time step) in order to make the time series stationary. Differencing involves the subtraction of the current values of a series with its previous values <span style='color:lightgreen; font-weight: bolder'>d number of times</span>.\n\n * :blue[MA(q) Moving Average] – a model that uses the dependency between an observation and a residual error from a moving average model applied to lagged observations. A moving average component depicts the error of the model as a combination of previous error terms. The order <span style='color:cyan; font-weight: bolder'>q represents the number of terms to be included in the model</span>. ",unsafe_allow_html=True)
